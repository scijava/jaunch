...
fun main() {
    ...
    val jvmArgs = arrayOf(
        ...
        "-Djfoo=jbar",
        "-Djava.class.path=" + classPath.joinToString(":")
    )
    val mainClassName = "sc.fiji.Main"
    val mainArgs = arrayOf("-Dmfoo=mbar")
...
@OptIn(ExperimentalForeignApi::class)
fun Array<String>.toCValuesRef(scope: MemScope): CValuesRef<CPointerVar<ByteVar>> {
    val arrayPtr = nativeHeap.allocArray<CPointerVar<ByteVar>>(size + 1)
    forEachIndexed { index, element ->
        arrayPtr[index] = element.cstr.getPointer(scope)
    }
    arrayPtr[size] = null
